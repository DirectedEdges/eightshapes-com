...
def create_njk_from_medium(medium_url)


  # doc = Nokogiri::XML(open(medium_url),&:noblanks)
  response = open(medium_url)
  # puts response.base_uri
  doc = Nokogiri::HTML(response.read,&:noblanks)
  medium_url = response.base_uri.to_s.gsub(/\?source=.*/, '')
  ...
  filename = filename.gsub(/https:\/\/medium.com\/@kevinmpowell\//, '')
